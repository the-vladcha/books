...
class BooksApiTestCase(APITestCase):
    ...
    def test_get(self):
        ...
        books = Book.objects.all().annotate(
            ...
            discount_price=F('price') * (100 - F('discount_book')) / 100,
        ...
        self.assertEqual(serializer_data[0]['discount_price'], '25.00')
    ...
    def test_get_ordering(self):
        url = reverse('book-list')
        response = self.client.get(url, data={'ordering': ('price', 'name')})
        books = Book.objects.all().annotate(
            annotated_likes=Count(Case(When(userbookrelation__like=True, then=1))),
            # rating=Avg('userbookrelation__rate'),
            discount_price=F('price') * (100 - F('discount_book')) / 100,
    ...
    def test_get_detail(self):
        ...
        expected_data = {
            "id": self.book_1.id,
            "name": self.book_1.name,
            "price": f"{self.book_1.price:.2f}",
            "author_name": self.book_1.author_name,
            "annotated_likes": 1,
            "rating": '5.00',
            "discount_price": f"{(self.book_1.price * (100 - self.book_1.discount_book) / 100):.2f}",
            'owner_name': self.book_1.owner.username,
            'readers': [
                {
                    'first_name': '',
                    'last_name': ''
...
class BooksRelationTestCase(APITestCase):
    def setUp(self):
        ...
        self.user2 = User.objects.create(username='test_username2')
        self.book_1 = Book.objects.create(name='Test book 1', price=25, author_name='Author 1', owner=self.user)
        self.book_2 = Book.objects.create(name='Test book 2', price=100, author_name='Author 5')
